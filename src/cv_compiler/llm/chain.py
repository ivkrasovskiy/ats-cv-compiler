...
class AgentChainProvider:
    ...
    def _run_agent(self, prompt: str, *, timeout: int) -> str:
        ...
        Claude CLI writes to the TTY when stdout is a pipe in plain text mode, so we use
        ``claude -p --output-format json`` which reliably writes JSON to stdout.
        Other CLIs (gemini, etc.) write to stdout normally with ``-p``.
        """
        ...
        for attempt in range(1, len(_AGENT_RETRY_DELAYS) + 2):
            ...
            if cfg.command == "claude":
                cmd = [cfg.command, "-p", prompt, "--output-format", "stream-json", "--verbose"]
                stdin_data = None
            elif cfg.prompt_mode == "arg":
                cmd = [cfg.command, *cfg.args, prompt]
                stdin_data = None
            else:
            ...
            output = _extract_claude_json_result(raw) if cfg.command == "claude" else raw
            if not output:
...
def _extract_claude_json_result(raw: str) -> str:
    """Extract the model's text from ``claude -p --output-format stream-json`` stdout.

    The CLI streams one JSON object per line.  Lines with ``type == "assistant"``
    carry the actual model content in ``message.content[].text``.  We collect and
    join all such text blocks.
    """
    chunks: list[str] = []
