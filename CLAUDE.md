...
### Code quality rules (enforced by `docs/code_metadata.md`)

- **No file may exceed 500 lines.** If an edit would push a file over 500 lines, split it first.
- **Before editing any file**, check `docs/code_metadata.md` for that file's "Imported by" list —
  those files may also need changes.
- **After completing code edits**, `docs/code_metadata.md` is auto-regenerated by the `Stop` hook.
  You can also run manually: `make metadata`
- To refresh the filesystem tree: `make layout`

### Checking metadata before editing

```
# Quick lookup — who imports app/backend/api/agent.py?
grep -A5 "app/backend/api/agent.py" docs/code_metadata.md
```
